;(() => {
    const queryString = window.location.search; //This gets the query string from the current URL, creates a URLSearchParams object, and retrieves the id parameter.
    const urlParams = new URLSearchParams(queryString);
    const id = urlParams.get("id");
    console.log(id);

    //This fetches the Pokémon data from pokedex.json and converts it to a JavaScript object.
    fetch("../pokedex.json")
        .then((rawData) => rawData.json()) //convert to json
        .then((pokedex) => {
            console.log(pokedex[id]);


            //extract specific details ID, name, image, description, types, species, height, weight, and abilities from the Pokémon object and generate in HTML.
            ...
            //updates the HTML elements on the page with the retrieved Pokémon data.
            ...
            //This calculates the stats and percentage values, updates the HTML elements, and animates the stat bars.
            // Stats section
